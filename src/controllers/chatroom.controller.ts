    ...
    try {
        ...
        if (type === 'personal') {
            ...
            const existingChat = await Chatroom.findOne({
                type: 'personal',
                'members.userId': {
                    $all: [req.user!._id, new mongoose.Types.ObjectId(memberIds[0])]
                }
            });
        ...
        chatroom.members.forEach((member: any) => {
...
export const updateChatroom = async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
...
export const addMembers = async (req: AuthRequest, res: Response) => {
    try {
        ...
        chatroom.members.push(...(newMembers as any));
        await chatroom.save();
